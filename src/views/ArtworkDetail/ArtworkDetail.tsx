...
function ArtworkDetail() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.isAlreadySaved) {
        Alert.alert('Artwork Already Saved', response.message, [{ text: 'OK' }]);
      } else {
        Alert.alert('Artwork Saved', response.message, [{ text: 'OK' }]);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDelete = async () => {
    try {
      await removeArtworkStorage(params.id);

      Alert.alert('Artwork Deleted', 'The artwork has been deleted from your saved artworks', [{ text: 'OK' }]);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <View style={styles.description}>
            ...
            <Animated.View style={[styles.divider, { opacity: textFadeAnimOrigin }]}>
              ...
              {artwork.artist_title && (
                <Text style={styles.headings}>
                  Artist: <Text style={styles.value}> {artwork.artist_title} </Text>
                </Text>
              )}
              {artwork.date_display && (
                <Text style={styles.headings}>
                  Date: <Text style={styles.value}> {artwork.date_display} </Text>
                </Text>
              )}
              {artwork.place_of_origin && (
                <Text style={styles.headings}>
                  Place of origin: <Text style={styles.value}> {artwork.place_of_origin} </Text>
                </Text>
              )}
            </Animated.View>
          )}
          ...
          {(artwork.dimensions ||
            ...
            <Animated.View style={[styles.divider, { opacity: textFadeAnimMaterials }]}>
              ...
              {artwork.dimensions && (
                <Text style={styles.headings}>
                  Dimensions: <Text style={styles.value}> {artwork.dimensions} </Text>
                </Text>
              )}
              {artwork.artwork_type_title && (
                <Text style={styles.headings}>
                  Type: <Text style={styles.value}> {artwork.artwork_type_title} </Text>
                </Text>
              )}
              {artwork.department_title && (
                <Text style={styles.headings}>
                  Department: <Text style={styles.value}> {artwork.department_title} </Text>
                </Text>
              )}
              {artwork.material_titles && artwork.material_titles.length > 0 ? (
                <Text style={styles.headings}>
                  Material: <Text style={styles.value}> {artwork.material_titles} </Text>
                </Text>
              ) : null}
            </Animated.View>
          )}
          ...
          {(artwork.publication_history || artwork.exhibition_history) && (
            <Animated.View style={[styles.divider, { opacity: textFadeAnimHistory }]}>
              ...
              {artwork.publication_history && (
                <Text style={styles.headings}>
                  Publication history: <Text style={styles.value}> {artwork.publication_history} </Text>
                </Text>
              )}
              {artwork.exhibition_history && (
                <Text style={styles.headings}>
                  Exhibition history: <Text style={styles.value}> {artwork.exhibition_history} </Text>
                </Text>
              )}
            </Animated.View>
          )}
